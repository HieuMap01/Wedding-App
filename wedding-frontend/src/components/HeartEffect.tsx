...
export default function HeartEffect() {
    ...
    const addHeart = (x: number, y: number) => {
        ...
        setHearts((prev) => [...prev.slice(-10), newHeart]); // Cap at 10 hearts for better performance
    ...
    const handleInteraction = (e: any) => {
        ...
        if (x !== undefined && y !== undefined) {
            // Spawn 2 hearts for a subtler but still visible effect
            addHeart(x, y);
            setTimeout(() => addHeart(x, y), 50);
        }
    };
    ...
    useEffect(() => {
        ...
        const throttleAdd = (e: any) => {
            const now = Date.now();
            if (now - lastSpawn > 400) { // Throttle back to 400ms manually
                handleInteraction(e);
                lastSpawn = now;
            }
        };
